...
class CppLevel1Hard extends StatefulWidget {
  const CppLevel1Hard({super.key});

  @override
  State<CppLevel1Hard> createState() => _CppLevel1HardState();
}

class _CppLevel1HardState extends State<CppLevel1Hard> {
  ...
  int score = 3;
  int remainingSeconds = 240; // Hard: 4 minutes
  ...
  Future<void> _loadGameConfig() async {
    try {
      ...
      final response = await ApiService.getGameConfigWithDifficulty('C++', 'Hard', 1);

      print('🔍 HARD GAME CONFIG RESPONSE:');
  ...
  void _initializeGameFromConfig() {
    ...
    try {
      print('🔄 INITIALIZING HARD GAME FROM CONFIG');
  ...
  String _getDefaultHint() {
    return "💡 Hint: Recursive functions need a base case. Make sure to handle n <= 1 and call the function recursively.";
  }
  ...
  void startGame() {
    ...
    print('🎮 HARD GAME STARTED - Initial Score: $score, Timer: $timerDuration seconds');
    startTimers();
  }
  ...
  Future<void> saveScoreToDatabase(int score) async {
    ...
    try {
      ...
      if (response['success'] == true) {
        ...
        print('✅ HARD SCORE SAVED SUCCESSFULLY');
      } else {
        print('❌ FAILED TO SAVE SCORE: ${response['message']}');
      }
    } catch (e) {
      print('❌ ERROR SAVING HARD SCORE: $e');
    }
  }
  ...
  Future<void> loadScoreFromDatabase() async {
    ...
    try {
      ...
      print('Error loading hard score: $e');
    }
  }
  ...
  // IMPROVED: Better answer checking logic similar to Medium level
  void checkAnswer() async {
    ...
    // IMPROVED ANSWER CHECKING - Similar to Medium level approach
    ...
    if (gameConfig != null) {
      ...
      // Fallback check for essential components in Hard Level 1
      bool hasFunctionDeclaration = droppedBlocks.any((block) => block.toLowerCase().contains('calculatefactorial'));
      bool hasBaseCase = droppedBlocks.any((block) => block.toLowerCase().contains('n <= 1'));
      bool hasRecursiveCall = droppedBlocks.any((block) => block.toLowerCase().contains('calculatefactorial(n - 1)'));
      bool hasFunctionCall = droppedBlocks.any((block) => block.toLowerCase().contains('calculatefactorial(5)'));
      bool hasOutput = droppedBlocks.any((block) => block.toLowerCase().contains('cout'));

      isCorrect = hasFunctionDeclaration && hasBaseCase && hasRecursiveCall && hasFunctionCall && hasOutput;

      print('✅ FALLBACK CHECK: $isCorrect');
      print('   hasFunctionDeclaration: $hasFunctionDeclaration');
      print('   hasBaseCase: $hasBaseCase');
      print('   hasRecursiveCall: $hasRecursiveCall');
      print('   hasFunctionCall: $hasFunctionCall');
      print('   hasOutput: $hasOutput');
    }
        ...
        builder: (_) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              if (score == 3)
                Text(
                  "🎉 Perfect! You've unlocked Level 2 Hard!",
              ...
              Container(
                ...
                color: Colors.black,
                child: Text(
                  _expectedOutput.isNotEmpty ? _expectedOutput : 'Factorial: 120',
                  style: TextStyle(
                    color: Colors.white,
            ...
            TextButton(
              onPressed: () {
                ...
                if (score == 3) {
                  ...
                  Navigator.pushReplacementNamed(context, '/cpp_level2_hard');
                } else {
  ...
  // UPDATED: Better organized code preview with proper structure - SAME AS MEDIUM
  ...
  // UPDATED: Organized code preview with proper structure - SAME AS MEDIUM
  ...
  Widget _buildSyntaxHighlightedLine(String code, int lineNumber) {
    ...
    if (code.trim().startsWith('#include')) {
      textColor = Color(0xFFCE9178); // Preprocessor - orange
        ...
        code.contains('return') || code.contains('int calculateFactorial')) {
      textColor = Color(0xFF569CD6); // Keywords and functions - blue
  ...
  Widget build(BuildContext context) {
    if (isLoading) {
      return Scaffold(
        appBar: AppBar(
          title: Text("⚡ C++ Hard - Level 1", style: TextStyle(fontSize: 18)),
          backgroundColor: Colors.red,
        ),
        body: Container(
          decoration: BoxDecoration(
          ...
          child: Center(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 20),
                Text(
                  "Loading Hard Game Configuration...",
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text("⚡ C++ Hard - Level 1", style: TextStyle(fontSize: 18 * _scaleFactor)),
        backgroundColor: Colors.red,
        actions: gameStarted
            ? [
          Padding(
          ...
          children: [
            ...
            if (levelCompleted)
              Padding(
                ...
                child: Column(
                  children: [
                    Text(
                      "✅ Level 1 Hard completed with perfect score!",
            ...
            Container(
              ...
              child: Column(
                children: [
                  Text(
                    gameConfig?['objective'] ?? "🎯 Hard Level 1 Objective",
                    style: TextStyle(fontSize: 18 * _scaleFactor, fontWeight: FontWeight.bold, color: Colors.red[800]),
                    textAlign: TextAlign.center,
                  ),
                  SizedBox(height: 10 * _scaleFactor),
                  Text(
                    gameConfig?['objective'] ?? "Complete the C++ Hard Level 1 challenge with functions and recursion",
                    textAlign: TextAlign.center,
                    style: TextStyle(fontSize: 14 * _scaleFactor, color: Colors.red[700]),
                  ),
                  SizedBox(height: 10 * _scaleFactor),
                  Text(
                    "🎁 Get a perfect score (3/3) to unlock Level 2 Hard!",
                    textAlign: TextAlign.center,
                    style: TextStyle(
                        fontSize: 12 * _scaleFactor,
                        color: Colors.purple,
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            isTagalog
                ? (gameConfig?['story_tagalog'] ?? 'Ito ay Hard Level 1 ng C++ programming! Pinakamahirap na hamon sa functions at recursion.')
                : (gameConfig?['story_english'] ?? 'This is C++ Hard Level 1! The ultimate challenge with functions and recursion.'),
