...
class _CppLevel2State extends State<CppLevel2> {
  ...
  void resetBlocks() {
    // Reduced blocks for C++ variables and output
    allBlocks = [
      'int x',
      '=',
      '5;',
  ...
  Future<void> saveScoreToPrefs(int score) async {
    ...
    await prefs.setInt('C++_level2_score', score);
    // Only mark as completed if score is perfect
    if (score == 3) {
      await prefs.setBool('C++_level2_completed', true);
      setState(() {
        level2Completed = true;
      });
    } else {
      await prefs.setBool('C++_level2_completed', false);
      setState(() {
  ...
  Future<void> loadScoreFromPrefs() async {
    final prefs = await SharedPreferences.getInstance();
    // FIXED: Use consistent key format with LevelSelectionScreen
    final savedScore = prefs.getInt('C++_level2_score');
    final completed = prefs.getBool('C++_level2_completed') ?? false;
  ...
  void checkAnswer() async {
    ...
    String normalizedCorrect = 'intx=5;cout<<"value:"<<x;'.toLowerCase();
      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text("Great job with C++ variables and output!"),
              SizedBox(height: 10),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("💻 C++ - Level 2: Variables & Output"),
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: () async {
              final prefs = await SharedPreferences.getInstance();
              // FIXED: Use consistent key format with LevelSelectionScreen
              await prefs.remove('C++_level2_completed');
              await prefs.remove('C++_level2_score');
              setState(() {
                level2Completed = false;
  ...
  Widget buildGameUI() {
    return SingleChildScrollView(
      ...
      Text(
        isTagalog
            ? 'Ngayon, natututo si Zeke tungkol sa mga variables sa C++! Gusto niyang ideklara ang isang variable at ipakita ang value nito. Pwede mo ba siyang tulungan?'
            : 'Now, Zeke is learning about variables in C++! He wants to declare a variable and display its value. Can you help him?',
        textAlign: TextAlign.justify,
        style: TextStyle(fontSize: 16),
      ),
      SizedBox(height: 20),
      Text('🧩 Arrange the puzzle blocks to declare a variable and output its value:',
