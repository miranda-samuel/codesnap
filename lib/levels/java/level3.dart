...
class _JavaLevel3State extends State<JavaLevel3> {
  ...
  void resetBlocks() {
    // Reduced blocks for if-else statement
    allBlocks = [
      'int age',
      '=',
      '18;',
  ...
  Future<void> saveScoreToPrefs(int score) async {
    ...
    await prefs.setInt('Java_level3_score', score);
    // Only mark as completed if score is perfect
    if (score == 3) {
      await prefs.setBool('Java_level3_completed', true);
      setState(() {
        level3Completed = true;
      });
    } else {
      await prefs.setBool('Java_level3_completed', false);
      setState(() {
  ...
  Future<void> loadScoreFromPrefs() async {
    final prefs = await SharedPreferences.getInstance();
    final savedScore = prefs.getInt('Java_level3_score');
    final completed = prefs.getBool('Java_level3_completed') ?? false;
  ...
  void checkAnswer() async {
    ...
    String normalizedCorrect = 'intage=18;if(age>=18){system.out.println("adult");}else{system.out.println("minor");}'.toLowerCase();
      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text("Great job with Java conditionals!"),
              SizedBox(height: 10),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("☕ Java - Level 3: Conditionals"),
      ...
      child: Column(
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: () async {
              final prefs = await SharedPreferences.getInstance();
              await prefs.remove('Java_level3_completed');
              await prefs.remove('Java_level3_score');
              setState(() {
                level3Completed = false;
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            isTagalog
                ? 'Ngayon, natututo si Zeke tungkol sa mga conditional statements sa Java! Gusto niyang gumawa ng program na magsasabi kung ang isang tao ay adult o minor batay sa kanilang edad. Pwede mo ba siyang tulungan?'
                : 'Now, Zeke is learning about conditional statements in Java! He wants to create a program that tells if a person is an adult or minor based on their age. Can you help him?',
            textAlign: TextAlign.justify,
            style: TextStyle(fontSize: 16),
          ),
          SizedBox(height: 20),
          Text('🧩 Arrange the puzzle blocks to form an if-else statement that checks if age is 18 or above:',
