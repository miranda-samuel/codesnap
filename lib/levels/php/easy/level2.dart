...
class _PhpLevel2State extends State<PhpLevel2> {
  ...
  String _instructionText = '🧩 Arrange the blocks to create a PHP variable and display it';
  List<String> _codeStructure = [];
  String _expectedOutput = 'My name is: John';
  ...
  Future<void> _loadGameConfig() async {
    try {
      ...
      final response = await ApiService.getGameConfig('PHP', 2);

      print('🔍 PHP LEVEL 2 GAME CONFIG RESPONSE:');
      ...
      print('❌ Error loading PHP Level 2 game config: $e');
      setState(() {
  ...
  void _initializeGameFromConfig() {
    ...
    try {
      print('🔄 INITIALIZING PHP LEVEL 2 GAME FROM CONFIG');
  ...
  String _getDefaultHint() {
    return "💡 Hint: PHP variables start with \$ symbol. Use . (dot) to concatenate strings!";
  }
  ...
  void startGame() {
    ...
    print('🎮 PHP LEVEL 2 GAME STARTED - Initial Score: $score, Timer: $timerDuration seconds');
    startTimers();
  }
  ...
  Future<void> saveScoreToDatabase(int score) async {
    ...
    try {
      print('💾 SAVING PHP LEVEL 2 SCORE:');
      ...
      if (response['success'] == true) {
        ...
        print('✅ PHP LEVEL 2 SCORE SAVED SUCCESSFULLY TO DATABASE');
      } else {
        print('❌ FAILED TO SAVE SCORE: ${response['message']}');
      }
    } catch (e) {
      print('❌ ERROR SAVING PHP LEVEL 2 SCORE: $e');
    }
  }
  ...
  Future<void> loadScoreFromDatabase() async {
    ...
    try {
      ...
      print('Error loading PHP Level 2 score: $e');
    }
  }
  ...
  bool isIncorrectBlock(String block) {
    ...
    // Default incorrect blocks for PHP Level 2
  ...
  void checkAnswer() async {
    ...
    if (gameConfig != null) {
      ...
      // Fallback check for PHP Level 2
      String expected = '\$name="john";echo"mynameis:".\$name;';
      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text("Excellent work! You've mastered PHP variables!"),
              SizedBox(height: 10),
  ...
  List<Widget> _buildOrganizedCodePreview() {
    ...
    for (int i = 0; i < _codeStructure.length; i++) {
      ...
      if (line.contains('// Define a variable') || line.contains('// Display the variable')) {
  ...
  Widget _buildSyntaxHighlightedLine(String code, int lineNumber) {
    ...
    if (code.trim().startsWith('<?php') || code.trim().startsWith('?>')) {
      ...
      textColor = Colors.white; // Semicolons - white
    }
  ...
  Widget build(BuildContext context) {
    if (isLoading) {
      return Scaffold(
        appBar: AppBar(
          title: Text("🐘 PHP - Level 2", style: TextStyle(fontSize: 18)),
          ...
          child: Center(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 20),
                Text(
                  "Loading PHP Level 2 Configuration...",
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            isTagalog
                ? (gameConfig?['story_tagalog'] ?? 'Ngayon ay matututo tayo ng PHP variables! Gamitin ang \$ symbol para sa variables.')
                : (gameConfig?['story_english'] ?? 'Now let\'s learn PHP variables! Use the \$ symbol for variables and . for concatenation.'),
