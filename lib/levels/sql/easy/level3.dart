...
class _SqlLevel3State extends State<SqlLevel3> {
  ...
  String _instructionText = '🧩 Arrange the blocks to form a SQL query with ORDER BY clause';
  List<String> _codeStructure = [];
  String _expectedOutput = 'Sorted Employee Data';
  ...
  Future<void> _loadGameConfig() async {
    try {
      ...
      final response = await ApiService.getGameConfig('SQL', 3);

      print('🔍 SQL LEVEL 3 GAME CONFIG RESPONSE:');
      ...
      print('❌ Error loading SQL Level 3 config: $e');
      setState(() {
  ...
  void _initializeGameFromConfig() {
    ...
    try {
      print('🔄 INITIALIZING SQL LEVEL 3 FROM CONFIG');
  ...
  List<String> _getDefaultCodeStructure() {
    return [
      "-- SQL SELECT Query with ORDER BY clause",
  ...
  List<Map<String, dynamic>> _getDefaultTableData() {
    return [
      ...
      {'id': 6, 'name': 'Sofia Martinez', 'department': 'IT', 'salary': 55000},
    ];
  }
  ...
  String _getDefaultHint() {
    return "💡 Hint: SQL ORDER BY clause syntax: SELECT * FROM table_name ORDER BY column_name ASC|DESC;";
  }
  ...
  void startGame() {
    ...
    print('🎮 SQL LEVEL 3 STARTED - Initial Score: $score, Timer: $timerDuration seconds');
    startTimers();
  }
  ...
  Future<void> saveScoreToDatabase(int score) async {
    ...
    try {
      print('💾 SAVING SQL LEVEL 3 SCORE:');
      ...
      if (response['success'] == true) {
        ...
        print('✅ SQL LEVEL 3 SCORE SAVED SUCCESSFULLY TO DATABASE');
      } else {
        print('❌ FAILED TO SAVE SCORE: ${response['message']}');
      }
    } catch (e) {
      print('❌ ERROR SAVING SQL LEVEL 3 SCORE: $e');
    }
  }
  ...
  Future<void> loadScoreFromDatabase() async {
    ...
    try {
      ...
      print('Error loading SQL Level 3 score: $e');
    }
  }
  ...
  bool isIncorrectBlock(String block) {
    ...
    // Default incorrect blocks for SQL Level 3
  ...
  void checkAnswer() async {
    ...
    if (gameConfig != null) {
      ...
      // Fallback check for SQL Level 3
      String expected = "select*fromemployeesorderbysalarydesc;";
    ...
    if (isCorrect) {
      ...
      showDialog(
        context: context,
        builder: (_) => AlertDialog(
          title: Text("✅ Correct SQL Query with ORDER BY Clause!"),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text("Excellent work! You've mastered SQL ORDER BY clause!"),
              SizedBox(height: 10),
              Text("Your Score: $score/3", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.green)),
              SizedBox(height: 10),
              if (score == 3)
                Text(
                  "🎉 Perfect! You've completed SQL Intermediate Level!",
          ...
          actions: [
            TextButton(
              ...
              child: Text(score == 3 ? "Back to Levels" : "Retry"),
            )
          ],
        ),
      );
  ...
  Widget _buildQueryResultPreview() {
    // Sort table data by salary in descending order
    List<Map<String, dynamic>> sortedData = List.from(_tableData);
    sortedData.sort((a, b) => (b['salary'] as int).compareTo(a['salary'] as int));
  ...
  List<Widget> _buildSQLCodePreview() {
    ...
    if (droppedBlocks.isEmpty) {
      codeLines.add(
        Text(
          '-- Drag SQL blocks here to build your query with ORDER BY clause...',
          style: TextStyle(
  ...
  Widget build(BuildContext context) {
    if (isLoading) {
      return Scaffold(
        appBar: AppBar(
          title: Text("🗃️ SQL - Level 3", style: TextStyle(fontSize: 18)),
          ...
          child: Center(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 20),
                Text(
                  "Loading SQL Level 3 Configuration...",
        ...
        child: Column(
          ...
          children: [
            ElevatedButton.icon(
              ...
              label: Text(gameConfig != null ? "Start Level 3" : "Config Missing", style: TextStyle(fontSize: 16 * _scaleFactor)),
              ...
              Padding(
                ...
                child: Column(
                  children: [
                    ...
                    Text(
                      "You've completed SQL Intermediate Level!",
            ...
            Container(
              ...
              child: Column(
                children: [
                  Text(
                    gameConfig?['objective'] ?? "🎯 SQL Level 3 Objective",
                    style: TextStyle(fontSize: 18 * _scaleFactor, fontWeight: FontWeight.bold, color: Colors.green[800]),
                    textAlign: TextAlign.center,
                  ),
                  SizedBox(height: 10 * _scaleFactor),
                  Text(
                    gameConfig?['objective'] ?? "Learn SQL ORDER BY clause to sort data from database tables in ascending or descending order",
                    textAlign: TextAlign.center,
                    style: TextStyle(fontSize: 14 * _scaleFactor, color: Colors.green[700]),
                  ),
                  SizedBox(height: 10 * _scaleFactor),
                  Text(
                    "📚 New Concept: ORDER BY Clause",
                    textAlign: TextAlign.center,
                    style: TextStyle(
                        fontSize: 14 * _scaleFactor,
                        color: Colors.blue,
                  ...
                  SizedBox(height: 10 * _scaleFactor),
                  Text(
                    "🎁 Get a perfect score (3/3) to complete SQL Intermediate Level!",
                    textAlign: TextAlign.center,
                    style: TextStyle(
                        fontSize: 12 * _scaleFactor,
                        color: Colors.purple,
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Flexible(
                child: Text('📖 SQL Level 3 Story', style: TextStyle(fontSize: 16 * _scaleFactor, fontWeight: FontWeight.bold, color: Colors.white)),
              ),
              TextButton.icon(
                onPressed: () {
                  final musicService = Provider.of<MusicService>(context, listen: false);
          ...
          Text(
            isTagalog
                ? (gameConfig?['story_tagalog'] ?? 'Ngayon ay matututo tayo ng ORDER BY clause! Paano mag-sort ng data base sa column.')
                : (gameConfig?['story_english'] ?? 'Now we learn ORDER BY clause! How to sort data based on specific columns in SQL.'),
